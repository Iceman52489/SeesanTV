...
module.exports.findByID = function(req, res, next) {
  ...
  Program.findOne({ id: programID }, 'id categoryID cover title updatedAt', function(err, query) {
    if(!err) {
      query.clips = [];

      Clip.find({ programID: programID }, 'id name src updatedAt', function(err, clips) {
        if(!err) {
          program = {
            id: query.id,
