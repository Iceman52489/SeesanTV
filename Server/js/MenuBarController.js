function MenuBarController(documentLoader, documentURL, loadingDocument) {
  ...
  documentLoader.fetch({
    ...
    success: function(menuBarDocument) {
      ...
      if(menuBarDocument.getElementsByTagName('menuBar').length) {
...
d('MenuBarController.selectMenuItem() -> ' + existingDocument);
  if(!existingDocument) {
    const controllerOptions = resolveControllerFromElement(menuItemElem);
d(JSON.stringify(controllerOptions));
d('isInitialItem ->' + isInitialItem);
...
d('MenuBarController.selectMenuItem -> handleDocument() -> Triggered');
        if(isInitialItem) {
